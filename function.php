<?php  function ConvertKhongDau($str) {
 ...
 * Link : https://drive.google.com/file/d/1MZCerAG2bnWplrf1pC6yaN0Afsriqpzb/view?usp=sharing
 */
 ...
 * log
 */
define('_access_log_file', 'access.log');
define('_error_log_file', 'error.log');

function LogAccess(){
    $time = date('Y-m-d H:i:s');
    $file = fopen(_access_log_file, 'a') or die('Loi khong ghi duoc AccessLog');

    $url = $_SERVER['REQUEST_URI'];
    $ip = $_SERVER['REMOTE_ADDR'];

    $content  = $ip.' '. $time .'# '.$url.PHP_EOL;

    $kq = fwrite($file, $content);
    fclose($file);

    if(!$kq)
    {
        die("Loi qua trinh ghi log access");
    }
}
...
function LogError($msg){
    ...
    $time = date('Y-m-d H:i:s');
    $file = fopen(_error_log_file, 'a') or die('Loi khong ghi duoc Log');

    // $url = $_SERVER['REQUEST_URI'];
    $ip = $_SERVER['REMOTE_ADDR'];

    $content  = $ip.' '. $time .'# '.$msg .PHP_EOL;
